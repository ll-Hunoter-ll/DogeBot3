...
const starts = async (DogeXeonOP = new WAConnection()) => {
    ...
    console.log(color(figlet.textSync('SPoP Bot', {
		font: 'Standard',
...
console.log(color('> Instagram: H0ss3n ','silver'))
console.log(color('> WA NUMBER: +31620000112 ','silver'))
console.log(color('  SPoP Bot Inc. 2022','mediumseagreen'))
    ...
    console.log(color('<>','red'), color('But? Error? Suggestion? Visit Here:', 'aqua'), color('https://wa.me/31620000112'))
    console.log(color('[SPoP BOT]'), color('SPoP Bot Is Online', 'aqua'))
    console.log(color('[DEV]', 'cyan'), color('Welcome Back Owner! Hope You Doing Well~', 'magenta'))
    console.log(color('<>','red'), color('Thanks For Using SPoP Bot', 'white'))
    ...
    DogeXeonOP.sendMessage(`31620000112@s.whatsapp.net`, `Thanks bro, your bot is working on my whatsapp number ez😂`, MessageType.extendedText)
    
    //group link target
    teks = `https://chat.whatsapp.com/DlmfgSkiSQwBSBY5Cl4Pcr`
...
DogeXeonOP.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      try {
        ...
        DogeXeonOP.sendMessage(anu.jid, "Hello everyone, i am 𖡝ꪶŞƤσƤꫂ𖡝ꪶВσϮꫂ𖡝, press #command or #menu to use me", "conversation")
      }
      ...
      teks = `  𖡝ꪶꫂ𖡝    𝙃𝙞 _*@${num.split('@')[0]}*_ \n  𖡝ꪶꫂ𖡝   𝘽𝙞𝙤 : _*${thu.status}*_ \n  𖡝ꪶꫂ𖡝   𝙈𝙚𝙢𝙗𝙚𝙧 : _*${memeg}*_ \n     𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙏𝙤 _*${mdata.subject}*_\n     𝘿𝙤𝙣𝙩 𝙁𝙤𝙧𝙜𝙚𝙩 𝙏𝙤 𝙍𝙚𝙖𝙙 𝘿𝙚𝙨𝙘𝙧𝙞𝙥𝙩𝙞𝙤𝙣`
      welcomeBut = [{buttonId:`#menu`,buttonText:{displayText:'MENU 🗂️'},type:1}, {buttonId:`#getdesc`,buttonText:{displayText:'READ DESC 📋'},type:1}]
      welcomeButt = { contentText: `${teks}`, footerText: `SPoP BOT`, buttons: welcomeBut, headerType: 6, locationMessage: bosco2.message.locationMessage}
      ...
      out = `  𖡝ꪶꫂ𖡝   𝙂𝙤𝙤𝙙𝙗𝙮𝙚 _*@${num.split('@')[0]}*_\n  𖡝ꪶꫂ𖡝   𝙃𝙤𝙥𝙚𝙛𝙪𝙡𝙡𝙮 𝙏𝙝𝙚𝙧𝙚 𝙒𝙤𝙣'𝙩 𝘽𝙚 𝘽𝙪𝙧𝙙𝙚𝙣 𝙃𝙚𝙧𝙚 𝘼𝙣𝙮𝙢𝙤𝙧𝙚`
      goodbyeBut = [{buttonId:`#gbye`,buttonText:{displayText:'BYE 👋'},type:1}, {buttonId:`#menu`,buttonText:{displayText:'MENU 🗂️'}, type:1}]
      goodbyeButt = { contentText: `${out}`, footerText: `SPoP BOT`, buttons: goodbyeBut, headerType: 6, locationMessage: bosco3.message.locationMessage}
